...
def playerturn(enemyheal):
    ...
    if "Dirty Rag" and "Glock 19" in inventory:
        question = input("How would you like to attack?\n1. Stab with a stick\n2. Whack With a baguette\n3. Throw dirty rag\n4. Shoot Glock 19")
        if question == "1":
            print ("You attacked the enemy.")
            rand (power * .05)
        ...
        if question == "4":
            print("You attacked the enemy!")
            enemyheal -= 9999999999999999999999999999999
            return enemyheal
...
def shop(temp):
    ...
    question = input("Shopkeeper: you wannna trade me something or buy something?")
    if question == "buy":
        if rand == 1:
        ...
        if rand == 2:
            question = input("\nYou can purchase a RNG Crate(50 Baguettes)or an RNG Flintlock(25 Baguettes). What do you want to buy?")
            if question == "Crate":
                if Inventory[baguette] >= 75:
...
while True:
    ...
    if massMurders == 11:
        ...
        if question == "explore":
            ...
            if rand == 2:
                time.sleep(0.3)
                print("You recieved a Glock 19! Now you can slaughter groups of zombies in 0.0003 seconds.")
                addInventory("Glock 19")
            if rand == 3:
